  ...
  return (
    <aside className="min-h-0 overflow-y-auto border-l">
      <header className="border-b px-5 py-4">
        ...
        <p className="mt-1 text-xs text-zinc-500">{state.status}</p>
      </header>
        ...
        {state.toolCalls.map((tool) => (
          <div key={tool.id} className="rounded-xl border p-3">
            ...
            {tool.input !== undefined && (
              <pre className="mt-3 overflow-x-auto rounded-lg bg-zinc-100 p-2 text-xs">
                {JSON.stringify(tool.input, null, 2)}
              </pre>
            )}
            {tool.output !== undefined && (
              <pre className="mt-2 overflow-x-auto rounded-lg bg-zinc-100 p-2 text-xs">
